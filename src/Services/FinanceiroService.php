...
class FinanceiroService
{
     ...
     * ============================
     */
    public function BuscaTiruloPorId(string $tituloId): FinTitulo
    {
        $titulo = $this->em->getRepository(FinTitulo::class)
            ->find($tituloId);

        if (!$titulo) {
            throw new DomainException('Título financeiro não encontrado.');
        }

        return $titulo;
    }

    public function EditarTitulo(string $id, array $data): void
    {
        $titulo = $this->BuscaTiruloPorId($id);

        if (in_array($titulo->getStatus(), ['PAGO', 'CANCELADO'])) {
            throw new DomainException('Não é possível editar títulos pagos ou cancelados.');
        }

        // Apenas campos editáveis
        if (isset($data['data_vencimento'])) {
            $titulo->setDataVencimento(new \DateTimeImmutable($data['data_vencimento']));
        }

        if (isset($data['valor'])) {
            $titulo->setValor((float) $data['valor']);
        }

        if (isset($data['observacoes'])) {
            $titulo->setObservacoes($data['observacoes']);
        }
    ...
    public function alterarTitulo(string $id, array $dados): void
    {
        $conn = $this->em->getConnection();
        ...
        try {
            ...
            if ($titulo->getValorPago() > 0) {
                throw new DomainException(
                    'Título com pagamento não pode ser alterado.'
                );
            }

            // identifica parcelado
            [$num, $total] = explode('/', $titulo->getParcela());

            $parcelado = ((int)$total) > 1;
            ...
            if (!empty($dados['valor'])) {
                ...
                if ($parcelado) {
                    throw new DomainException(
                        'Não é permitido alterar valor de título parcelado.'
                    );
                }

                $titulo->setValor(
                    $this->money($dados['valor'])
                );
            }
     ...
     * ============================
     */
    public function cancelarTitulo(string $id): void
    {
        $titulo = $this->BuscaTiruloPorId($id);

        if ($titulo->getValorPago() > 0) {
            throw new DomainException(
                'Não é possível cancelar título com pagamento.'
            );
        }

        if ($titulo->getStatus() === 'CANCELADO') {
            throw new DomainException('Título já cancelado.');
        }
     ...
     * ============================
     */
    public function cancelarDocumento(string $numeroDocumento): void
    {
        $titulos = $this->em->getRepository(FinTitulo::class)->findBy([
        ...
        foreach ($titulos as $titulo) {
            if ($titulo->getValorPago() > 0) {
                throw new DomainException(
                    'Documento possui parcelas pagas.'
                );
            }
        }
