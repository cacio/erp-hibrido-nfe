...
class FinanceiroController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->participanteService = new ParticipanteService($this->em);
        $this->planoContaService = new PlanoContaService($this->em);
    }

    private function repo()
    {
        return $this->em->getRepository(FinTitulo::class);
    }
    /**
     * LISTAGEM
     */
    ...
    public function edit(string $id): void
    {
        try {
            ...
            // 1️⃣ Busca o título (regra no Service)
            $titulo = $this->financeiro->BuscaTiruloPorId($id);

            // 2️⃣ Só títulos em aberto podem ser editados
            if ($titulo->getStatus() !== 'ABERTO') {
                $this->flash(
                    'error',
                    'Somente títulos em aberto podem ser editados.'
                );
                $this->redirect('/financeiro');
                return;
            }

            // 3️⃣ Listas auxiliares para a view
            $participantes = $this->participanteService->listarAtivosPorTenant($tenantId);
            $planos        = $this->planoContaService->listarAnaliticos();

            // 4️⃣ Renderiza a view edit.php
            $this->render('/financeiro/edit', [
                'titulo'        => $titulo,
    ...
    {
        try {
            $this->financeiro->alterarTitulo($id, $_POST);
            $this->flash('success', 'Título alterado com sucesso.');
            $this->redirect('/financeiro');
        } catch (\DomainException $e) {
            $this->flash('error', $e->getMessage());
