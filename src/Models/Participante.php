...
class Participante
{
    ...
    public function getEnderecoPrincipal(): ?array
    {
        if (!$this->enderecoJson) {
            return null;
        }

        $data = is_array($this->enderecoJson)
            ? $this->enderecoJson
            : json_decode($this->enderecoJson, true);

        return $data['principal'] ?? null;
    }

    public function getUfPrincipal(): ?string
    {
        $endereco = $this->getEnderecoPrincipal();
        return $endereco['uf'] ?? null;
    }
