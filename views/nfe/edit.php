...
<div class="top-bar">
    <div style="display: flex; align-items: center; gap: 15px;">
        <a href="/nfe" class="btn btn-outline">⬅️ Voltar</a>
...
<form id="nfe-form" method="POST" action="/nfe/<?= $nfe->getId() ?>" data-id="<?= $nfe->getId() ?>">
            ...
            <div class="stat-card">
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
                    ...
                    <div class="input-group">
                        ...
                        <input type="datetime-local" name="data_emissao" class="form-control" value="<?= $nfe->getDataEmissao()->format('Y-m-d H:i:s') ?>">
                    </div>
                    <div class="input-group">
                        <label>Natureza da Operação</label>
                        <input type="text" name="natureza_operacao" value="<?= $nfe->getNaturezaOperacao() ?>" class="form-control">
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', carregarNfe);

    function carregarNfe() {
        const vendaId = document.getElementById('nfe-form').dataset.id;

        fetch(`/nfe/${vendaId}/dados`)
            .then(r => r.json())
            .then(data => {
                if (data.error) {
                    alert(data.error);
                ...
                hidratarDestinatario(data.destinatario);
                hidratarTransportadora(data.transportadora);
                hidratarItens(data.itens);
            });
    }

    function hidratarDestinatario(d) {
        if (!d) return;

        document.getElementById('cliente-nome').innerText = d.nome;
        document.getElementById('cliente-doc').innerText = d.documento;
        document.getElementById('cliente-endereco').innerText =
            `${d.municipio}/${d.uf}`;

        document.getElementById('cliente-info-box').style.display = 'block';
        document.getElementById('cliente_busca').parentElement.style.display = 'none';
        document.getElementById('cliente_id').value = d.id;

    }

    function hidratarTransportadora(t) {
        if (!t) return;

        document.getElementById('transp-nome').innerText = t.nome;
        document.getElementById('transp-doc').innerText = t.documento;
        document.getElementById('transp-info-box').style.display = 'block';

        document.getElementById('transportadora_busca').parentElement.style.display = 'none';
        document.getElementById('transportadora_id').value = t.id;

        document.getElementById('transp_propria').checked = false;
        toggleTransportadora();
    }


    function hidratarItens(lista) {
        itens = [];
...
<script>
    ...
    function salvarDestinatario(participanteId, force = false) {
        const vendaId = document.getElementById('nfe-form').dataset.id;

        fetch(`/nfe/${vendaId}/destinatario`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            .then(res => {
                if (res.confirm) {
                    if (confirm(res.message)) {
                        salvarDestinatario(participanteId, true);
                    }
                    return;
                }
...
<script>
    ...
    function salvarItem() {
        ...
        fetch(`/nfe/${vendaId}/itens`, {
            ...
            .then(res => {
                ...
                adicionarItemTabela(res.item);
                atualizarTotais(res.item.impostos);
                closeModal('modal-item')
            });
    }

    function adicionarItemTabela(itemApi) {
        const item = {
            id: itemApi.id,
            produto: itemApi.produto,
            qtd: parseFloat(itemApi.quantidade),
            vlrUnit: parseFloat(itemApi.valor_unitario),
            cfop: itemApi.cfop,
            ncm: itemApi.ncm,
            total: parseFloat(itemApi.total),
            impostos: itemApi.impostos // 🔑 AQUI
        };
    ...
    function renderizarItens() {
        ...
        let noItems = document.getElementById('no-items-row');

        // Garante que a linha exista
        if (!noItems) {
            noItems = document.createElement('tr');
            noItems.id = 'no-items-row';
            noItems.innerHTML = `
        ...
        // Remove apenas linhas de itens (não a mensagem)
        container.querySelectorAll('tr[data-item]').forEach(tr => tr.remove());
        ...
        itens.forEach((item, index) => {
            ...
            <td>${item.ncm}</td>
            <td>${item.cfop}</td>
            <td>${item.qtd.toFixed(4)}</td>
            <td>R$ ${item.vlrUnit.toLocaleString('pt-BR', { minimumFractionDigits: 4 })}</td>
            <td><strong>R$ ${item.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</strong></td>
            <td>
                <button type="button"
                        class="btn btn-ghost"
                        style="color: #ef4444;"
                        onclick="removerItem('${item.id}')">🗑️</button>
            </td>
        `;
...
<script>
    function atualizarTotais() {
        ...
        itens.forEach(item => {
            ...
            if (item.impostos) {
                totalIcms += item.impostos.icms?.valor || 0;
                totalPis += item.impostos.pis?.valor || 0;
                totalCofins += item.impostos.cofins?.valor || 0;
            }
        });
        ...
        document.getElementById('total-icms').innerText =
            totalIcms.toLocaleString('pt-BR', {
                style: 'currency',
                currency: 'BRL'
            });

        document.getElementById('total-pis-cofins').innerText =
            (totalPis + totalCofins).toLocaleString('pt-BR', {
                style: 'currency',
                currency: 'BRL'
            });
